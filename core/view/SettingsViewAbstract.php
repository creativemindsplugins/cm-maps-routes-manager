...
namespace com\cminds\mapsroutesmanager\view;
...
abstract class SettingsViewAbstract {
	...
	public function renderCategory($category) {
		...
		return '<div class="settings-category settings-category-'. $category .'">'. apply_filters('cmmrm-settings-category', $result, $category) .'</div>';
	}
	...
	public function renderSubcategory($category, $subcategory) {
		...
		return '<div class="settings_'. $category .'_'. $subcategory .'">'. apply_filters('cmmrm-settings-subcategory', $result, $category, $subcategory) .'</div>';
	}
	...
	protected function renderMultiCheckboxItem($name, $value, $label, $currentValue = null) {
		...
		$style = ''; if($value == 'overview_path') { $style = ' style="display:none;"'; }
		return sprintf('<div'.$style.'><label><input type="checkbox" name="%s[]" value="%s"%s /> %s</label></div>',
			esc_attr($name),
			esc_attr($value),
