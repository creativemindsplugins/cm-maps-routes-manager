...
if ($total_pages > 1): ?>
	...
	$writeItem = function($pageNumber, $label = null, $class = '', $disabled = false) use ($total_pages, $page, $base_url) {
		...
		if ($pageNumber == $page) $class[] = 'cmmrm-pagination-current';
		if ($disabled) {
			$class[] = 'cmmrm-pagination-disabled';
			$template = '<li%s>%s</li>';
		} else {
			if ($pageNumber > 1) {
...
// 					$url = trailingslashit($base_url) . 'page/' . $pageNumber .'/';
	...
	$writeItem($page-1, '&laquo;', 'cmmrm-pagination-prev', $page == 1);
	...
	$writeItem($page+1, '&raquo;', 'cmmrm-pagination-next', $page == $total_pages);
