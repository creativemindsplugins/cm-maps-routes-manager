...
class AttachmentController extends Controller {
	...
	static function cmmrm_location_after_save(Location $location, $i) {
		if( isset($_POST['locations']['images'][$i]) ) {
			$location->setImages(sanitize_text_field($_POST['locations']['images'][$i]));
		} else {
			$location->setImages(array());
		}
		if( isset($_POST['locations']['icon'][$i]) ) {
			$location->setIcon(sanitize_text_field($_POST['locations']['icon'][$i]));
		} else {
			$location->setIcon('');
		}
		if( isset($_POST['locations']['icon_size'][$i]) ) {
			$location->setIconSize(sanitize_text_field($_POST['locations']['icon_size'][$i]));
		} else {
	...
	static function cmmrm_get_image_id() {
		...
		if (!empty($_POST['url'])) {
			if (Attachment::isYouTubeUrl( sanitize_text_field($_POST['url']) )) { // YouTube
				$attachment = Attachment::createYouTube(0, sanitize_text_field($_POST['url']));
			} else {
