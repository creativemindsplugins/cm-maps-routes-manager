...
// 		$_GET['msg'] = 'route_save_success';
		if (!empty($_GET['msg'])) {
			$out .= static::getMessageView( sanitize_text_field($_GET['msg']) );
		}
	...
	static protected function processSaveRoute() {
		...
		if (isset($_GET['id'])) {
			$route = Route::getInstance( sanitize_text_field($_GET['id']) );
		} else {
			$route = new Route();
...
// 					var_dump(memory_get_usage());flush();ob_flush();
					...
					$location->setContent( sanitize_text_field($_POST['locations']['description'][$i]) );
					...
					if ($id) {
						...
						$location->setLocationType( sanitize_text_field($_POST['locations']['type'][$i]) );
	...
	static function processDeleteRoute() {
		if (isset($_GET['id']) AND $route = Route::getInstance( sanitize_text_field($_GET['id']) ) AND $route->canDelete()) {
	...
	static function cmmrm_route_params_save() {
		if (!empty($_POST['nonce']) AND wp_verify_nonce($_POST['nonce'], self::UPDATE_PARAMS_NONCE)) {
			if (!empty($_POST['routeId']) AND $route = Route::getInstance( sanitize_text_field($_POST['routeId']) )) {
				if (!empty($_POST['distance'])) $route->setDistance( sanitize_text_field($_POST['distance']) );
				if (!empty($_POST['duration'])) $route->setDuration( sanitize_text_field($_POST['duration']) );
				if (!empty($_POST['minElevation'])) $route->setMinElevation( sanitize_text_field($_POST['minElevation']) );
				if (!empty($_POST['maxElevation'])) $route->setMaxElevation( sanitize_text_field($_POST['maxElevation']) );
				if (!empty($_POST['elevationGain'])) $route->setElevationGain( sanitize_text_field($_POST['elevationGain']) );
				if (!empty($_POST['elevationDescent'])) $route->setElevationDescent( sanitize_text_field($_POST['elevationDescent']) );
				if (!empty($_POST['avgSpeed'])) $route->setAvgSpeed( sanitize_text_field($_POST['avgSpeed']) );
				if (!empty($_POST['locations']) AND is_array($_POST['locations'])) {
					foreach ($_POST['locations'] as $data) {
						if ($location = Location::getInstance( sanitize_text_field($data['id']) )) {
							$location->setAddress( sanitize_text_field($data['addr']) );
						}
						Location::clearInstances();
					}
				}
